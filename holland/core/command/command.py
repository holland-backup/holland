...
#                    help="Show help")
PARSER.add_argument('-v', '--verbose', action='store_const', const='info',
                    dest='log_level',
...
class Command(object):
    ...
    def __init__(self):
        if LooseVersion(argparse.__version__) < LooseVersion('1.4.0'):
            self.optparser = SUBPARSER.add_parser(self.name,
                                                  help="%s %s" % (self.name, self.description),
                                                  description=self.name)
            self.alias_parser = []
            for alias in self.aliases:
                self.alias_parser.append(SUBPARSER.add_parser(alias, help="Alias to %s" % self.name,
                                                              description=self.name))
            ...
            self.optparser = SUBPARSER.add_parser(self.name,
                                                  help="%s %s" % (self.name, self.description),
                                                  aliases=self.aliases, description=self.name)
    ...
    def dispatch(self, opts, args):
        ...
        try:
            ...
            LOGGER.error("Failed comamnd %s': %s",
                         self.optparser.prog, ex)
            return os.EX_SOFTWARE
        except BaseException as ex:
            LOGGER.error("Uncaught exception while running command '%s': %r",
                         self.optparser.prog, ex, exc_info=True)
            return os.EX_SOFTWARE
