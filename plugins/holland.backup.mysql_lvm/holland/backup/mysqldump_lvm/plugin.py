...
from holland.backup.mysqldump_lvm.util import connect_simple, build_snapshot, \
                                          setup_actions, setup_mysqldump
...
class MysqlLVMBackup(object):
    ...
    def backup(self):
        ...
        setup_actions(snapshot=snapshot,
                      config=self.config,
                      client=self.client)
        setup_mysqldump(snapshot, self.mysqldump_plugin, snap_datadir)
