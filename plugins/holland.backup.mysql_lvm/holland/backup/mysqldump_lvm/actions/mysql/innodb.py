...
class InnodbRecoveryAction(object):
    def __init__(self, mysqld_exe, mysqld_config):
    ...
    def __call__(self, event, snapshot_fsm, snapshot):
        ...
        mysqld = MySQLServer(self.mysqld_exe, my_conf)
        ...
        while mysqld.poll() is None:
            ...
            time.sleep(0.5)
        LOG.info("%s has stopped", self.mysqld_exe)
        ...
        if mysqld.returncode != 0:
            ...
            raise BackupError("%s exited with non-zero status (%s) during "
                              "InnoDB recovery" % (self.mysqld_exe, mysqld.returncode))
        else:
            LOG.info("%s ran successfully", self.mysqld_exe)
