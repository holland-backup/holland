...
def _cleanup():
    for inst in _active[:]:
        if inst._internal_poll(_deadstate=sys.maxint) >= 0: # pragma: no cover
            try:
                _active.remove(inst)
            except ValueError:
...
def _demo_posix(): # pragma: no cover
    #
...
def _demo_windows(): # pragma: no cover
    #
